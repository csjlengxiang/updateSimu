...
namespace BLL
{
    public class PositionService
    {
        ...
        public class Station
        {
            ...
            public Station(string id, string name, string jw, string jbm)
            {
                ID = id;
                Name = name;
        ...
        private void FullStations()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (dr["ZM"].ToString() == "" || dr["JD"].ToString() == "" || dr["WD"].ToString() == "" || dr["DBM"].ToString() == "未发现" || dr["TIMIS"].ToString() == "未发现" || dr["ZMLM"].ToString() == "未发现")
                    continue;
                stations.Add(new Station(dr["CZID"].ToString() ,dr["ZM"].ToString(), dr["JD"].ToString() + "," + dr["WD"].ToString(), dr["JBM"].ToString()));
                cnt++;
            }
            //MessageBox.Show(cnt.ToString());

        }
        public Station GetNear(double j, double w, ref string stationName)
        {
